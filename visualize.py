...
def convert(parameter):
    ...
    parameter_name = re.search(r"^(Location|User Group|Item Policy)", parameter).group(
        1
    )

    operator = re.search(
        r"^(Location|User Group|Item Policy)\s*(=|In List|Is Empty|Is Not Empty|Not Equals|Not In List|Contains)",
        parameter,
    ).group(2)
...
def sort_rows_by_lowest_integer_prefix_safe(pivot_table):
    sort_values = pivot_table.apply(
        lambda row: min(
            [
                extract_and_convert_row_number_prefix_safe(val)
                for val in row
                if extract_and_convert_row_number_prefix_safe(val) is not None
            ],
            default=float("inf"),
        ),
        axis=1,
...
for y in range(0, len(files)):
    ...
    df_full = pd.read_excel(
        filename,
        usecols=["Parameter", "Due Date"] + columns_ix_names,
        engine="openpyxl",
    )
    ...
    writer = pd.ExcelWriter(
        oDir + "/" + filename_base + " - Visualized.xlsx", engine="xlsxwriter"
    )
    for column in columns_ix_names:
        df_full[f"{column}_with_row"] = df_full.index.to_series().apply(
            lambda x: f"{x+1}: "
        ) + df_full[column].astype(str)
        ...
        pivot_table_sorted_both = sort_columns_by_lowest_integer_prefix_safe(
            pivot_table_sorted_rows
        )
