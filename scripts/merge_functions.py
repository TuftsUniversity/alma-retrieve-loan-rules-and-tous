...
def mergeAndPresent(rule_df):
    ...
    rule_df["Parameter"] = (
        rule_df["Name"].astype("string")
        + " "
        + rule_df["Operator"].astype("string")
        + " "
        + rule_df["Value"].astype("string")
    )
    ...
    rule_df["Parameter"] = rule_df.groupby(columns_to_apply_list_master, dropna=False)[
        "Parameter"
